...
<html lang="en">
  <head>
    ...
    <title>Dosen | ARMY University</title>
  </head>
  <body>
    ...
    <div class="row no-gutters mt-5">
      <div class="col-md-2 bg-dark mt-2 pr-3 pt-4">
        <ul class="nav flex-column ml-3 mb-5">
          ...
          <li class="nav-item">
            <a class="nav-link text-white" href="dosen.php"><i class="fas fa-chalkboard-teacher mr-2"></i> Dosen</a><hr class="bg-secondary">
          </li>
          <li class="nav-item">
      ...
      <div class="col-md-10 p-5 pt-4">
        <h3><i class="fas fa-chalkboard-teacher mr-2"></i> DOSEN</h3><hr>
            ...
            <table class="table table-bordered">
              ...
              <tbody>
                ...
                if($_SERVER['REQUEST_METHOD'] == "POST") {
                  ...
                  if($cari != ''){
                    $sql      = "SELECT * FROM m_dosen where nama like '%$cari%'";
                    $query    = $sql;
                    $queryJml = $sql;
                  } else {
                    $query    = "SELECT * FROM m_dosen LIMIT $posisi, $batas";
                    $queryJml = "SELECT * FROM  m_dosen";
                    $no       = $posisi + 1;
                  }
                } else {
                  $query    = "SELECT * FROM m_dosen LIMIT $posisi, $batas";
                ...
                $sql_dosen = mysqli_query($koneksi, $query) or die (mysqli_error($koneksi));
                if(mysqli_num_rows($sql_dosen) > 0) {
                  while($d = mysqli_fetch_array($sql_dosen)) {
                ?>
                ...
                <tr>
                  ...
                  <td><?php echo $d['jk']; ?></td>
                  <td><?php echo $d['kd_jabatan_dosen']; ?></td>
                  <td><?php echo $d['status']; ?></td>
                  <td>
                    <a class="btn btn-warning ml-2" href="edit.php?kd_dosen=<?php echo $d['kd_dosen']; ?>" role="button"><i class="far fa-edit"></i></a> 
                  </td>
                  <td>
                    <a class="btn btn-danger ml-2" href="hapus.php?kd_dosen=<?php echo $d['kd_dosen']; ?>" role="button"><i class="fas fa-trash"></i></a>
                  </td>
                </tr>
                <?php 
                  }
          ...
          <div class="page mt-5 mx-auto">
            <nav aria-label="Page navigation example">
              <ul class="pagination justify-content-end">
                ...
                <li class="page-item">
                  <a class="page-link" <?php if($hal < $jml_hal) {
                    echo "href='?hal=$next'"; } ?>>Next</a>
                </li>
              </ul>
            </nav>
          </div>
